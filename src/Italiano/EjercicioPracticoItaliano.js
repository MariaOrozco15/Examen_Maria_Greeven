...
// Componente funcional ItalianQuizScreen
const ItalianQuizScreen = () => {
  ...
  const handleSubmit = () => {
    const results = Object.entries(answers).map(([questionId, answer]) => {
      ...
      if (questionId === '1' && compareAnswer(answer, 'Buongiorno')) {
        isCorrect = true;
      }
      if (questionId === '2' && compareAnswer(answer, 'Buonasera')) {
        isCorrect = true;
      }
      if (questionId === '3' && compareAnswer(answer, 'Buon pomeriggio')) {
        isCorrect = true;
      }
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Bienvenido a tus ejercicios de italiano</Text>
      {/* Pregunta 1 */}
...
// Estilos CSS utilizando StyleSheet.create
...
export default ItalianQuizScreen;
